"""The completeness failure is placement-driven, not shard-driven. Test whether raising K
(greedy candidates per panel, currently 16) makes placement reliably cover both walls."""
...
def evaluate(pc,mult,K,seed):
    ...
    return prim,cryptic(ts,table,r,panels,pT),cA,cB,nA+nB
CANDS=[(16,0.7,32),(16,0.7,64),(12,0.7,48)]
SEEDS=[1,2,3,4,5,6]; results={}
for pc,mult,K in CANDS:
    P=[];Cy=[];CP=[];T=[]
    print(f"\n--- panels={pc}, mult={mult}, K={K} ---")
    for s in SEEDS:
        prim,carry,cA,cB,tot=evaluate(pc,mult,K,s); mc=min(cA,cB)
        P.append(prim);Cy.append(carry);CP.append(mc);T.append(tot)
        print(f"  seed {s}: primary={prim:.3f} carry={carry:.3f} compA={cA:.0f}% compB={cB:.0f}% shards={tot} {'' if (mc>=85 and tot<=300) else '<-- FAILS'}")
    passed=sum(1 for c,t in zip(CP,T) if c>=85 and t<=300)
    results[f"{pc}_{mult}_K{K}"]=dict(primary=P,carry=Cy,comp=CP,total=T,passed=passed)
    print(f"  gate passes {passed}/6 | primary {st.mean(P):.3f}±{st.pstdev(P):.3f} carry {st.mean(Cy):.3f}±{st.pstdev(Cy):.3f} worst-comp {min(CP):.0f}%")
json.dump(results,open("out_thickness_test/search/seedsK.json","w"),indent=1)
print("\n=== reliably pass (>=5/6)? ===")
for k,v in results.items():
    print(f"  {k}: {'RELIABLE' if v['passed']>=5 else 'unreliable'} ({v['passed']}/6)")
