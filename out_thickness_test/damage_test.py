"""Cross-talk-aware shard->panel assignment. damage_weight=0 == old rng.choice (control).
Same fixed panel placement (wide/seed), 0.5x density. 10 seeds x 4 damage weights."""
...
SEEDS=list(range(1,11)); DW=[0.0,0.5,1.0,2.0]

def evaluate(panels,seed,dw):
    ...
    sc,op,fr,res,sd,bs,si=decompose.fragment_shards_overlap(ts,table,targets,names=names,
        white_thr=ts.white_threshold,max_stack=ts.color_max_stack,seed=seed,damage_weight=dw)
    gen=time.time()-t0
...
for dw in DW:
    for seed in SEEDS:
        ...
        acc,ct,comp,gen=evaluate(panels,seed,dw)
        r={"dw":dw,"seed":seed,"compA":comp["A"],"compB":comp["B"],"gen_s":gen,
        ...
        results.append(r)
        dq=" DQ(blank)" if comp["A"]<85 or comp["B"]<85 else ""
        print(f"dw={dw:<4} seed={seed:2d} | comp A={comp['A']:5.1f}% B={comp['B']:5.1f}% | "
              f"A_bad={r['A_bad']:5.1f}% B_bad={r['B_bad']:5.1f}% B_good={r['B_good']:5.1f}% | "
              f"A {acc['A']['rmse']:.3f}/{acc['A']['ssim']:.3f} B {acc['B']['rmse']:.3f}/{acc['B']['ssim']:.3f} "
              f"| {gen:.1f}s{dq}")

json.dump(results,open("out_thickness_test/damage_results.json","w"),indent=1)
print("\n=== SUMMARY (all 10 seeds; control = dw 0 == old rng.choice) ===")
print("Bar: beat control's B_bad stdev AND get most seeds <15%")
for dw in DW:
    sub=[r for r in results if r["dw"]==dw]
    bb=[r["B_bad"] for r in sub]
    tag=" (CONTROL)" if dw==0 else ""
    print(f"dw={dw}{tag}: B_bad mean={st.mean(bb):5.1f}% stdev={st.pstdev(bb):4.1f} "
          f"min={min(bb):5.1f}% max={max(bb):5.1f}% <15%: {sum(1 for x in bb if x<15)}/10")
    print(f"     B_good={st.mean(r['B_good'] for r in sub):5.1f}%  "
          f"A_rmse={st.mean(r['A_rmse'] for r in sub):.3f} A_ssim={st.mean(r['A_ssim'] for r in sub):.3f} "
          f"B_rmse={st.mean(r['B_rmse'] for r in sub):.3f} B_ssim={st.mean(r['B_ssim'] for r in sub):.3f} | "
          f"minComp={min(min(r['compA'],r['compB']) for r in sub):.1f}%  gen={st.mean(r['gen_s'] for r in sub):.1f}s")
print("\nwrote out_thickness_test/damage_results.json")
