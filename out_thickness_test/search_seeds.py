"""MULTI-SEED validation of the 3 candidates from the fine sweep. Flat portraits.
Candidates: (10,0.3) best-raw-primary, (12,0.3) balanced, (16,0.3) most-cryptic.
6 seeds each. Report mean AND stdev on primary (ssim+edge), cryptic (carry), completeness."""
...
for pc,mult in CANDS:
    ...
    print(f"\n--- panels={pc}, mult={mult} ---")
    for s in SEEDS:
        prim,carry,mincomp,tot=evaluate(pc,mult,s)
        P.append(prim);Cy.append(carry);Cp.append(mincomp);T.append(tot)
        print(f"  seed {s}: primary(ssim+edge)={prim:.3f}  carry={carry:.3f}  min-comp={mincomp:.0f}%  shards={tot}")
    results[f"{pc}_{mult}"]=dict(primary=P,carry=Cy,comp=Cp,total=T)
    print(f"  MEAN primary={st.mean(P):.3f} (sd {st.pstdev(P):.3f}) | "
          f"carry={st.mean(Cy):.3f} (sd {st.pstdev(Cy):.3f}) | "
          f"min-comp across seeds={min(Cp):.0f}% | shards={T[0]}")
json.dump(results,open("out_thickness_test/search/seeds.json","w"),indent=1)
print("\n=== VERDICT INPUT ===")
for k,v in results.items():
    print(f"{k}: primary {st.mean(v['primary']):.3f}±{st.pstdev(v['primary']):.3f}  "
          f"carry {st.mean(v['carry']):.3f}±{st.pstdev(v['carry']):.3f}  "
          f"worst-comp {min(v['comp']):.0f}%")
print("\nsaved seeds.json")
