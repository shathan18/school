"""Vermeers: gate + carry unreliable at K=16. Test K=32/48 for placement robustness."""
...
CANDS=[(20,0.5,32),(16,0.5,32),(20,0.7,32),(20,0.5,48)]
SEEDS=[1,2,3,4,5,6]; R={}
for pc,mult,K in CANDS:
    ...
    print(f"  gate {passed}/6 | primary {st.mean(P):.3f}±{st.pstdev(P):.3f} carry {st.mean(Cy):.3f}±{st.pstdev(Cy):.3f} worst-comp {min(CP):.0f}%")
json.dump(R,open("out_thickness_test/search/vermeer_K.json","w"),indent=1)
print("\n=== reliable (>=5/6)? ===")
for k,v in R.items():
    print(f"  {k}: {'RELIABLE' if v['passed']>=5 else 'unreliable'} ({v['passed']}/6) | carry {st.mean(v['carry']):.3f}±{st.pstdev(v['carry']):.3f}")
