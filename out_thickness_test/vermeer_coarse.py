"""Open-ended search on the two REAL Vermeer photos (front/back Girl with a Pearl Earring).
Base fixed: signed dw=0.5 cw=0.5 agree=0.90. Baseline cryptic reading + COARSE one-axis pass."""
...
A_IMG="examples/WhatsApp Image 2026-07-16 at 11.39.33.jpeg"        # front
B_IMG="examples/WhatsApp Image 2026-07-16 at 11.39.33 (1).jpeg"    # back
...
for w in ("A","B"):
    m=targets[w]; print(f"wall {w}: subject={subject[w].mean()*100:.0f}% of canvas  mean-sat={(m.max(-1)-m.min(-1)).mean():.3f}")
def cryptic(ts,table,r,panels,pT):
    prim={p.name:primary_wall_of(ts,table,p) for p in panels}; carry=[];se=[]
    for gi,p in enumerate(panels):
        w=prim[p.name]; pt=np.ones_like(pT); pt[gi]=pT[gi]; solo=r.render_color_np(pt)[w]
        d=(1.0-solo.mean(-1))>0.05; carry.append((d&subject[w]).sum()/max(subject[w].sum(),1))
        se.append(_metrics.edge_fidelity(solo,targets[w]))
    return max(carry),max(se)
def evaluate(cfg,seed=3):
    ...
    panels,_=build_panels_greedy(scene,count=cfg["panels"],mode="deliberate",K=cfg.get("K",16),targets=targets,
                                 seed=seed,standoff=cfg.get("standoff",0.5),anchor_range=cfg.get("anchor",(0.5,2.4)),**ab)
    ...
    return dict(total=nA+nB,A=nA,B=nB,compA=comp["A"],compB=comp["B"],ssim=ssim,edge=edge,carry=mc,soloedge=me,
                pass_=(comp["A"]>=85 and comp["B"]>=85 and nA+nB<=300))
BASE=dict(panels=8,angle=(5,85),standoff=0.5,anchor=(0.5,2.4),mult=1.0,K=16)
print("\n=== BASELINE (current-best-style) on REAL Vermeers ===")
b=evaluate(BASE)
print(f"  total={b['total']} (A{b['A']}/B{b['B']}) comp {b['compA']:.0f}/{b['compB']:.0f}% | SSIM {b['ssim']:.3f} edge {b['edge']:.3f} | carry={b['carry']:.3f} soloedge={b['soloedge']:.3f} | pass={b['pass_']}")
configs=[("BASE",BASE)]
for mu in (0.3,0.5,0.7): configs.append((f"shards mult={mu}",{**BASE,"mult":mu}))
for pc in (12,16): configs.append((f"panels={pc}",{**BASE,"panels":pc,"mult":0.7}))
configs.append(("panels=16 K=32",{**BASE,"panels":16,"mult":0.7,"K":32}))
for ang in [(20,70)]: configs.append((f"angle={ang}",{**BASE,"angle":ang,"mult":0.7}))
configs.append(("angle near-axis",{**BASE,"angle_bands":[(0,15),(75,90)],"mult":0.7}))
rows=[]
print("\n=== COARSE (1 seed) ===")
print(f"{'config':18s} | {'tot':>4s} {'cA/cB':>7s} | {'SSIM':>5s} {'edge':>5s} | {'carry':>5s} | pass")
for name,cfg in configs:
    r=evaluate(cfg); r["name"]=name; rows.append(r)
    print(f"{name:18s} | {r['total']:4d} {r['compA']:3.0f}/{r['compB']:<3.0f} | {r['ssim']:.3f} {r['edge']:.3f} | {r['carry']:.3f} | {'OK' if r['pass_'] else 'FAIL'}")
json.dump(rows,open("out_thickness_test/search/vermeer_coarse.json","w"),indent=1,default=str)
print("\nsaved")
