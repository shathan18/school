...
OUT = "out_thickness_test/bw3_final"
def b64(p):
...
HTML = f"""<title>High-Key Monochrome — Dual-Wall Shadow Reconstruction</title>
<style>
...
<div class="wrap">
  <p class="eyebrow">Dual-Wall Shadow Sculpture · High-Key Monochrome</p>
  <h1>Same portrait, softened to high key</h1>
  <p class="lede">The Girl front/back pair as <b>low-contrast, high-key</b> black &amp; white — pale, gentle,
  almost a soft-pencil rendering. It reconstructs <b>smoothly</b> (no blotchiness) and posts the highest
  SSIM of all four girl runs — but that top score is partly a <b>metric artifact</b>: a light, low-variance
  image is simply easy to match.</p>
  ...
  <p class="hint">Smooth and delicate, but <em>faint</em> — with little dark content there's little to reconstruct,
  so it reads as a gentle ghost of the figure. The face still doesn't resolve; the shard-count limit is unchanged.</p>

  <hr class="rule">
  <p class="h2">The finding</p>
  <div class="finding">
    <div class="bignum"><span class="n">{mean['ssimA']:.3f}</span>
      <div class="cap">Wall-A SSIM — the highest of all four girl runs, but read it with the caveat at left</div></div>
    <div>
      <p><b>The best SSIM here doesn't mean the best reconstruction.</b> A high-key image is mostly bright,
      low-variance pixels, and SSIM scores those generously — a faint reconstruction of a faint target
      lands high without carrying much. The more honest cross-variant number is <em>double duty</em>, and by
      that measure high-key sits mid-pack ({mean['bgA']:.1f}% Wall A):</p>
      <div class="cf">
        <span class="pill">colour <b>23.6%</b></span><span class="arrow">›</span>
        <span class="pill">mid-tone <b>18.6%</b></span><span class="arrow">›</span>
        <span class="pill">high-key <b>{mean['bgA']:.1f}%</b></span><span class="arrow">›</span>
        <span class="pill">hi-contrast <b>12.0%</b></span>
      </div>
    </div>
  </div>
...
{rows()}
  ...
  <div class="notes">
    <div class="note"><p class="k">Smooth beats blotchy</p>Where the high-contrast run clipped shards to pure
    black/white and looked scattered, high key keeps everything in a narrow bright band, so shard tones sit
    close together and the surface reads <b>smooth</b>. If you want a gentle, delicate object, this is the tonal
    end to aim for — at the cost of presence.</p>
    <div class="note"><p class="k">Faint, and still soft</p>Little dark content means little to reconstruct — the
    figure reads as a <b>ghost</b>. And as in every girl run the face doesn't resolve; that's the shard-count
    wall, untouched by tone. Grayscale is still a K-sheet-only monochrome build.</div>
  </div>

  <hr class="rule">
  <p class="h2">How it was made</p>
  <div class="method">
    <div class="mcard"><p class="k">Source pair</p><p class="v"><em>Girl with a Pearl Earring</em><br>front &amp; back, high-key grayscale</p></div>
  ...
  <div class="foot"><span>Interactive 3-D scene: <span class="mono">out_thickness_test/bw3_final/scene.html</span></span><span>Multi-seed validated · seed {BEST} shown</span></div>
</div>
<script>
