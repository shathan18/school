"""
FULL-QUALITY DELIVERABLE on the Girl-with-Pearl-Earring front/back pair (background removed,
so the solver reconstructs ONLY the girl). Current FIXED pipeline: signed-damage dw0.5/cw0.5,
colour-agreement credit (match_tol=0.30). Multi-seed -> pick best RMSE -> render + interactive
...
MATCH_TOL = 0.30
OUT = "out_thickness_test/girl_final"; os.makedirs(OUT, exist_ok=True)
...
SP = scene.solve   # pipeline default density
...
def run(seed):
    ...
    sc, op, fr, res, sd, bs, si = decompose.fragment_shards_overlap(
        ...
        max_stack=ts.color_max_stack, seed=seed, damage_weight=0.5, credit_weight=0.5, match_tol=MATCH_TOL)
...
SEEDS = [1, 2, 3, 4, 5]
print(f"pair: Girl front/back (bg removed)  |  signed dw0.5 cw0.5, colour-agree credit (match_tol {MATCH_TOL})\n")
...
for ri, w in enumerate(("A", "B")):
    ...
    ax[ri, 0].set_ylabel(["Wall A  (girl, front)", "Wall B  (girl, back)"][ri], fontsize=13)
ax[0, 0].set_title("SOURCE", fontsize=14, fontweight="bold")
ax[0, 1].set_title("RECONSTRUCTED SHADOW", fontsize=14, fontweight="bold")
plt.suptitle(f"Girl with a Pearl Earring — front/back, seed {r['seed']} ({r['shards']} shards, {r['used']}/14 panels)",
             fontsize=14, y=0.997)
